...
| 2a  | **Dictionary** (inside loop)  | Add six text fields: <br/> • `sample_type` → literal `dietary_energy_consumed` <br/> • `value` → Repeat Item → Get Details of Health Sample → **Value** <br/> • `unit` → Repeat Item → Get Details of Health Sample → **Unit** <br/> • `start_time` → Repeat Item → Get Details → **Start Date** → Format Date → **ISO 8601** with milliseconds (activate ISO 8601 Time toggle) <br/> • `end_time` → same as start_time but pick **End Date** <br/> • `source_name` → Repeat Item → Get Details → **Source** |
